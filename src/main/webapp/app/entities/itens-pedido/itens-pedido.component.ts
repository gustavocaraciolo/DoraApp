...
currentAccount: any;
    ...
    loadAll() {
        if (this.currentSearch) {
            this.itensPedidoService.search({
                page: this.page - 1,
    ...
    transition() {
        this.router.navigate(['/itens-pedido'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    trackId(index: number, item: ItensPedido) {
        return item.id;
    }
    registerChangeInItensPedidos() {
        this.eventSubscriber = this.eventManager.subscribe('itensPedidoListModification', (response) => this.loadAll());
    }
